...
bool Models::ObjectDetectionYolov2Model::updateLayerProperty(const InferenceEngine::CNNNetReader::Ptr net_reader)
{
  ...
  {
    slog::warn << "This model seems not Yolo-like! We got " << std::to_string(output_info_map.size())
                                                      ...
                                                      const float& confidence_thresh, const bool& enable_roi_constraint)
{
  try
  {
    ...
    const float* detections = request->GetBlob(output)
                                  ->buffer()
    ...
    for (unsigned int i = 0; i < raw_results.size(); ++i)
    {
      ...
      {
        auto iou =
            dynamic_vino_lib::ObjectDetection::calcIoU(raw_results[i].getLocation(), raw_results[j].getLocation());
        if (iou >= 0.45)
        {
