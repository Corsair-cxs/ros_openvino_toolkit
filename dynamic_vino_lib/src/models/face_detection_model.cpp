 ...
 */
/**
 * @brief a header file with declaration of FaceDetectionGazeModel class
...
Models::FaceDetectionGazeModel::FaceDetectionGazeModel(const std::string& model_loc, int max_batch_size)
  : FaceDetectionModel(model_loc, max_batch_size)
{
  slog::debug << "TESTING: in FaceDetectionGazeModel" << slog::endl;
  // addCandidatedAttr(std::make_shared<Models::SSDModelAttr>());
}

const std::string Models::FaceDetectionGazeModel::getModelCategory() const
{
  return "Object Detection SSD";
}

bool Models::FaceDetectionGazeModel::enqueue(const std::shared_ptr<Engines::Engine>& engine, const cv::Mat& frame,
...
bool Models::FaceDetectionGazeModel::matToBlob(const cv::Mat& orig_image, const cv::Rect&, float scale_factor,
...
bool Models::FaceDetectionGazeModel::fetchResults(const std::shared_ptr<Engines::Engine>& engine,
                                                   std::vector<dynamic_vino_lib::FaceDetectionResult>& results,
                                                   const float& confidence_thresh, const bool& enable_roi_constraint)
{
  ...
  for (int i = 0; i < max_proposal_count; i++)
  {
    ...
    dynamic_vino_lib::FaceDetectionResult result(r);
    std::string label =
...
bool Models::FaceDetectionGazeModel::updateLayerProperty(const InferenceEngine::CNNNetReader::Ptr net_reader)
{
