...
dynamic_vino_lib::LandmarksDetectionResult::LandmarksDetectionResult(const cv::Rect& location) : Result(location)
{
}

// LandmarksDetection
dynamic_vino_lib::LandmarksDetection::LandmarksDetection() : dynamic_vino_lib::BaseInference()
{
}

dynamic_vino_lib::LandmarksDetection::~LandmarksDetection() = default;
void dynamic_vino_lib::LandmarksDetection::loadNetwork(const std::shared_ptr<Models::LandmarksDetectionModel> network)
{
  valid_model_ = network;
  setMaxBatchSize(network->getMaxBatchSize());
}

bool dynamic_vino_lib::LandmarksDetection::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
  if (getEnqueuedNum() == 0)
  {
    results_.clear();
...
const dynamic_vino_lib::Result* dynamic_vino_lib::LandmarksDetection::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void dynamic_vino_lib::LandmarksDetection::observeOutput(const std::shared_ptr<Outputs::BaseOutput>& output)
{
  if (output != nullptr)
  {
...
dynamic_vino_lib::LandmarksDetection::getFilteredROIs(const std::string filter_conditions) const
{
