...
dynamic_vino_lib::PersonReidentificationResult::PersonReidentificationResult(const cv::Rect& location)
  : Result(location)
{
}
...
bool dynamic_vino_lib::PersonReidentification::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
  if (getEnqueuedNum() == 0)
  {
    results_.clear();
...
const dynamic_vino_lib::Result* dynamic_vino_lib::PersonReidentification::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void dynamic_vino_lib::PersonReidentification::observeOutput(const std::shared_ptr<Outputs::BaseOutput>& output)
{
  if (output != nullptr)
  {
...
dynamic_vino_lib::PersonReidentification::getFilteredROIs(const std::string filter_conditions) const
{
