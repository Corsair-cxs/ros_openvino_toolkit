...
// FaceDetectionResult
dynamic_vino_lib::FaceDetectionResult::FaceDetectionResult(const cv::Rect& location) : Result(location)
{
}
...
void dynamic_vino_lib::FaceDetection::loadNetwork(std::shared_ptr<Models::FaceDetectionModel> network)
{
  valid_model_ = network;

  setMaxBatchSize(network->getMaxBatchSize());
}
bool dynamic_vino_lib::FaceDetection::enqueue(const cv::Mat& frame, const cv::Rect& input_frame_loc)
{
...
const dynamic_vino_lib::FaceDetection::Result* dynamic_vino_lib::FaceDetection::getLocationResult(int idx) const
{
  return &(results_[idx]);
}
...
void dynamic_vino_lib::FaceDetection::observeOutput(const std::shared_ptr<Outputs::BaseOutput>& output)
{
  if (output != nullptr)
  {
...
dynamic_vino_lib::FaceDetection::getFilteredROIs(const std::string filter_conditions) const
{
...
dynamic_vino_lib::FaceDetectionResultFilter::FaceDetectionResultFilter()
{
}

void dynamic_vino_lib::FaceDetectionResultFilter::init()
{
...
void dynamic_vino_lib::FaceDetectionResultFilter::acceptResults(const std::vector<Result>& results)
{
  results_ = results;
}

std::vector<cv::Rect> dynamic_vino_lib::FaceDetectionResultFilter::getFilteredLocations()
{
...
bool dynamic_vino_lib::FaceDetectionResultFilter::isValidLabel(const Result& result, const std::string& op,
                                                                 const std::string& target)
{
  return stringCompare(result.getLabel(), op, target);
}

bool dynamic_vino_lib::FaceDetectionResultFilter::isValidConfidence(const Result& result, const std::string& op,
                                                                      const std::string& target)
{
  return floatCompare(result.getConfidence(), op, stringToFloat(target));
}

bool dynamic_vino_lib::FaceDetectionResultFilter::isValidResult(const Result& result)
{
  ISVALIDRESULT(key_to_function_, result);
}

double dynamic_vino_lib::FaceDetection::calcIoU(const cv::Rect& box_1, const cv::Rect& box_2)
{
